<template>
    <div class="ww-rich-text" :class="{ '-readonly': isReadonly, editing: isEditing }" data-capture>
        <template v-if="richEditor">
            <!-- Modern Modular Toolbar -->
            <ModularToolbar
                v-if="content.useModularToolbar && !hideMenu && !content.customMenu"
                :editor="richEditor"
                :config="content"
            ...
            <!-- Legacy Native Menu (backward compatible) -->
            <div class="ww-rich-text__menu native-menu" v-else-if="!hideMenu && !content.customMenu" :style="menuStyles">
                <!-- Texte type (normal, ...) -->
...
import ModularToolbar from './components/toolbar/ModularToolbar.vue';
import { SlashCommand } from './extensions/slash-command.js';
import { YouTube } from './extensions/youtube.js';
import { renderSlashMenu } from './components/slash-menu/slash-render.js';
import { getEnabledSlashCommands } from './components/slash-menu/slash-config.js';
...
export default {
    ...
    computed: {
        ...
        menuStyles() {
            ...
            return {
                ...
                '--menu-hover-bg': this.content.menuHoverBg || '#eee',
                '--menu-hover-icon-color': this.content.menuHoverIconColor || '#000000',
                'flex-wrap': this.content.wrapMenu ? 'wrap' : 'nowrap',
                'justify-content': justifyMap[this.content.menuPosition] || 'flex-start',
            };
        },
        richStyles() {
            return {
    ...
    methods: {
        ...
        loadEditor() {
            ...
            console.log('[Editor] Slash menu enabled:', this.content.enableSlashMenu);
            console.log('[Editor] Modular toolbar enabled:', this.content.useModularToolbar);
            ...
            this.richEditor = new Editor({
                ...
                extensions: [
                    ...
                    this.content.enableSlashMenu &&
                        SlashCommand.configure({
                            suggestion: {
                                char: '/',
                                items: ({ query }) => {
                                    console.log('[Slash] Items called with query:', query);
                                    const enabled = getEnabledSlashCommands(this.content);
                                    console.log('[Slash] Enabled commands:', enabled.length, enabled);
                                    if (!query || query === '/') {
                                        console.log('[Slash] Showing all commands');
                                        return enabled;
                                    }

                                    const searchTerm = query.toLowerCase().replace('/', '');
                                    const filtered = enabled.filter(cmd => {
                                        return (
                                    ...
                                    console.log('[Slash] Filtered commands for "' + searchTerm + '":', filtered.length);
                                    return filtered;
                                },
                                render: () => {
                                    console.log('[Slash] Render function called');
                                    return renderSlashMenu({
                                        onSelect: (command) => {
                                            console.log('[Slash] Command selected:', command);
                                        },
                                        onCancel: () => {
                                            console.log('[Slash] Menu cancelled');
                                        },
                                        getDarkMode: () => this.content.slashMenuDarkMode,
                                    });
                                },
                            },
                        }),
        ...
        handleToolbarAction({ action, args }) {
            // Handle toolbar actions from modular toolbar
            if (!this[action]) {
        ...
        insertInlineMath(latex = '') {
            // Placeholder for inline math - will be implemented when math extension is properly configured
            const expression = latex || window.prompt('Enter LaTeX expression:');
            if (expression) {
                console.log('Inline math:', expression);
                // Implementation depends on Mathematics extension configuration
            }
        },
        insertBlockMath(latex = '') {
            // Placeholder for block math - will be implemented when math extension is properly configured
            const expression = latex || window.prompt('Enter LaTeX expression:');
            if (expression) {
                console.log('Block math:', expression);
                // Implementation depends on Mathematics extension configuration
            }
        },
    },
...
.ww-rich-text {
    ...
    &__menu {
        ...
        select {
            ...
            &:hover {
                background-color: var(--menu-hover-bg, rgb(245, 245, 245));
                color: var(--menu-hover-icon-color, var(--menu-color));
            }
        }
        &-item {
            padding: 2px;
